...
The next six-case review batch applied five approvals and one rejection from existing cached evidence without new provider calls. `db_deadlift` retains the rejected candidate ID/name for provenance but has no approved media. `shoulder_press` is human-approved in concept but remains `needs_provider_id` and cannot resolve at runtime until the verified `Dumbbell Overhead Press` provider identity is available. Alternate variants remain review notes until a separate alternate schema is needed.

The final cached review batch removed `dead_bug` from the authoritative seed (the migration drops it from the local SQLite catalog on restart while the mapping record preserves rejection provenance) and approved the remaining reviewed names/variants. Entries with provider names but no cached provider ID remain `needs_provider_id` or acceptable reviewed metadata and are never runtime playback candidates until identity is confirmed.
